...
tff('constraint_71', axiom, ($lesseq(number(jumps), number(jumpsr)) | ~(nodup(jumpsr)) | ~(length(jumpsr) = $sum($sum(length_c2(mines10), length_c2(mines11)), 2)) | $lesseq(number(jumpsr), $sum(count(mines10), count(mines11))))).
