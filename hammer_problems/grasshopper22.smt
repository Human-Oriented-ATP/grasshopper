...
(assert (! (forall ((jsa t_jump_set) (jsc t_jump_set) (jsb t_jump_set)) (= (jumpset_merge (jumpset_merge jsa jsb) jsc) (jumpset_merge jsa (jumpset_merge jsb jsc)))) :named constraint-6))
...
(assert (! (forall ((jsa t_jump_set) (jsc t_jump_set) (jsb t_jump_set)) (= (subtract (jumpset_merge jsa jsb) (jumpset_merge jsa jsc)) (subtract_c1 jsb jsc))) :named constraint-49))
...
(assert (! (getitem mines01 0) :named constraint-73))
(assert (! (<= (length_c2 mines00) (length_c2 mines1)) :named constraint-74))
(assert (! (= mines1 (minefield_concat mines10 mines11)) :named constraint-75))
(assert (! (= (length_c2 mines10) (length_c2 mines00)) :named constraint-76))
(assert (! (forall ((x Int)) (or (not (getitem mines00 x)) (getitem mines_un x))) :named constraint-77))
(assert (! (forall ((x Int)) (or (not (getitem mines10 x)) (getitem mines_un x))) :named constraint-78))
(assert (! (or (not (<= (length_c2 mines10) (length_c2 mines00))) (= (length_c2 mines_un) (length_c2 mines00))) :named constraint-79))
(assert (! (or (not (<= (length_c2 mines00) (length_c2 mines10))) (= (length_c2 mines_un) (length_c2 mines10))) :named constraint-80))
(assert (! (<= (count mines00) (count mines_un)) :named constraint-81))
(assert (! (<= (count mines10) (count mines_un)) :named constraint-82))
(assert (! (<= (count mines_un) (+ (count mines00) (count mines10))) :named constraint-83))
(assert (! (= jumpsr (s jumps_ih)) :named constraint-84))
(assert (! (forall ((x Int)) (or (not (getitem (landings jumps_ih) x)) (and (not (getitem mines_un x)) (not (getitem mines11 (+ x (* (- 1) (length_c2 mines_un)))))))) :named constraint-85))
(assert (! (= jumps_ih (jumps_concat (jumps_singleton jumps_ih0) jumps_ihr)) :named constraint-86))
(assert (! (not (= (length (subtract (jumpset_merge (jumpset_merge (jumpset_singleton jumps_ih0) (jumpset_singleton jumps_max)) (s jumps_ihr)) jumps)) 0)) :named constraint-87))
