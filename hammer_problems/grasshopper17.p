...
tff('constraint_78', axiom, ($lesseq(number(jumps), number(jumpsr)) | ~(nodup(jumpsr)) | ~(length(jumpsr) = $sum($sum(length_c2(mines_un), length_c2(mines11)), 1)) | $lesseq(number(jumpsr), $sum(count(mines_un), count(mines11))))).
