...
tff('constraint_40', axiom, ![X:$int, Ma:t_mine_field, Mb:t_mine_field] : (getitem(minefield_concat(Ma, Mb), X) <=> (getitem(Ma, X) | getitem(Mb, $sum(X, $product(-1, length_c2(Ma))))))).
...
tff('constraint_75', axiom, jumpsr = s(jumps_ih)).
tff('constraint_76', axiom, ![X:$int] : (~(getitem(landings(jumps_ih), X)) | ~(getitem(mines1, X)))).
tff('constraint_77', axiom, ~(length(subtract(jumpset_merge(jumpset_singleton(jumps_max), s(jumps_ih)), jumps)) = 0)).
