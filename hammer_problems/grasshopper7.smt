...
(assert (! (not (= (count mines0) 0)) :named constraint-68))
(assert (! (= jumpsr (s jumps_ih)) :named constraint-69))
(assert (! (forall ((x Int)) (or (not (getitem (landings jumps_ih) x)) (not (getitem mines1 x)))) :named constraint-70))
(assert (! (or (= (+ boom (* (- 1) (length_c1 jumps_max)) 1) 0) (getitem (landings jumps_ih) (+ boom (* (- 1) (length_c1 jumps_max))))) :named constraint-71))
(assert (! (getitem mines boom) :named constraint-72))
