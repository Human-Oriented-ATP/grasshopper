...
(assert (! (forall ((x Int) (ma t_mine_field) (mb t_mine_field)) (= (getitem (minefield_concat ma mb) x) (or (getitem ma x) (getitem mb (+ x (* (- 1) (length_c2 ma))))))) :named constraint-40))
...
(assert (! (forall ((jsb t_jump_set) (jsa t_jump_set)) (= (subtract jsa (jumpset_merge jsa jsb)) empty_jumpset)) :named constraint-47))
(assert (! (forall ((jsb t_jump_set) (jsa t_jump_set)) (= (subtract (jumpset_merge jsa jsb) jsa) jsb)) :named constraint-48))
(assert (! (forall ((jsb t_jump_set) (jsa t_jump_set) (jsc t_jump_set)) (= (subtract (jumpset_merge jsa jsb) (jumpset_merge jsa jsc)) (subtract_c1 jsb jsc))) :named constraint-49))
...
(assert (! (not (<= (number jumpsr) (count mines1))) :named constraint-74))
(assert (! (= jumpsr (s jumps_ih)) :named constraint-75))
(assert (! (forall ((x Int)) (or (not (getitem (landings jumps_ih) x)) (not (getitem mines1 x)))) :named constraint-76))
(assert (! (not (= (length (subtract jumps (jumpset_merge (jumpset_singleton jumps_max) (s jumps_ih)))) 0)) :named constraint-77))
