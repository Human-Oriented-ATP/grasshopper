...
tff('constraint_2', axiom, ![Ja:t_jumps, Jc:t_jumps, Jb:t_jumps] : jumps_concat(jumps_concat(Ja, Jb), Jc) = jumps_concat(Ja, jumps_concat(Jb, Jc))).
...
tff('constraint_49', axiom, ![Jsc:t_jump_set, Jsb:t_jump_set, Jsa:t_jump_set] : subtract(jumpset_merge(Jsa, Jsb), jumpset_merge(Jsa, Jsc)) = subtract_c1(Jsb, Jsc)).
...
tff('constraint_84', axiom, ($lesseq(number(jumps), number(jumpsr)) | ~(nodup(jumpsr)) | ~(length(jumpsr) = $sum($sum(length_c2(mines_un), length_c2(mines11)), 1)) | $lesseq(number(jumpsr), $sum(count(mines_un), count(mines11))))).
